...
public class PostCreate {
	...
	public boolean insertPostInDatabase(int userId, int blogId) {
		...
		boolean status = true;
		// the blog object used to call this object needs to have its blogTitle,
		// blogPostTitle and blogPostBody parameters initialized before this
		...
		try {
			...
			pst = connectionManager.getConnection().prepareStatement(
					"insert into post values( 0, '" + blogId + "','"
							+ postTitle + "','" + postBody + "', curdate() )");
			pst.executeUpdate();
			// closing the connection to prepare for the next prepared
			// statement.
			pst.close();
			// select postid from post table where blogid and title is the same
			pst = connectionManager.getConnection().prepareStatement(
					"select postId from post where blogId = '" + blogId
							+ "' AND title = '" + postTitle + "' ");
			rs = pst.executeQuery();
			rs.first();
			...
			// insert postid and user id into user_post
			pst = connectionManager.getConnection().prepareStatement(
					"insert into user_post values('" + userId + "', '" + postId
							+ "') ");
			pst.executeUpdate();
			pst.close();
			// status = rs.next();
			...
			// errorMessage =
			// "Error with previous login attempt. Incorrect Username and Password.";
			status = false;
		} finally {
			// we now have to manage closing the connection a different way...at
			// logout...
			if (pst != null) {
				try {
					pst.close();
